...
if __name__ == '__main__':
    ...
    if not (input_type_single):
        ...
        plt.show()
            ...
            #r1=plt.bar(ind,mean_step_ge_time,width,color='r')
        r2=plt.bar(ind+width,initialestimator_nm_one_list,width,color='g')
        r3=plt.bar(ind+width*2,initialestimator_nm_con_one_list,width,color='b')
        r4=plt.bar(ind+width*3,second_nm_one_list,width,color='y')
        r5=plt.bar(ind+width*4,second_nm_con_one_list,width,color='m')
        plt.xticks([r+width*2 for r in range(N)],k_list)
        titlelabels = "Function: {}".format(get_label(function_number))
        plt.title(titlelabels)
        plt.ylabel("Number of one steps")
        plt.xlabel("k")
