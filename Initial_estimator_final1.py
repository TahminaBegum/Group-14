...
if __name__ == '__main__':
    ...
    """
        ...
        please write it is foe test one """
    ...
    if not (input_type_single):
        ...
        for k1 in np.arange(kr + 0.7, kr + 4.9, 0.7):
            ...
            for a in np.arange(-input_r, input_r, 0.1):
                ...
                if not (decide(f(x) == 0)):
                    ...
                    for j in range(len(all_method)):
                        if all_method[j] == 5:
                            t_start = time.process_time()
                            bound1_second_con, bound2_second_con, step_second_con = get_estimator_contactor_taylor(f, x,
                                                                                                                  counter,
                                                                                                                  k)
            ...
            mean_step_ie_con.append(
                np.array(initialestimator_nm_con)[np.nonzero(np.array(initialestimator_nm_con))].mean())
            ...
            mean_step_ie_time.append(
                np.array(initialestimator_nm_time)[np.nonzero(np.array(initialestimator_nm_time))].mean())
            mean_step_ie_con_time.append(
                np.array(initialestimator_nm_con_time)[np.nonzero(np.array(initialestimator_nm_con_time))].mean())
            mean_step_ie_second_time.append(np.array(second_nm_time)[np.nonzero(np.array(second_nm_time))].mean())
            mean_step_ie_second_con_time.append(
                np.array(second_nm_con_time)[np.nonzero(np.array(second_nm_con_time))].mean())
        ...
        if not (decide(f(x) == 0)):
            ...
            for j in range(len(all_method)):
                if all_method[j] == 2:
                    ...
                    if not (decide(bound1 == bound2)):
                        ...
                        print("The total steps for true root finding with initial estimator sign method(", bound2, "):= ", step)
                        t_end = time.process_time()
            ...
            for j in range(len(all_method)):
                if all_method[j] == 5:
                    t_start = time.process_time()
                    bound1_second_con, bound2_second_con, step_second_con = get_estimator_contractor_taylor(f, x, counter,
                                                                                                          k)
                    if not (decide(bound1_second_con == bound2_second_con)):
